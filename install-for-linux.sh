...
# Installs Nix (single user), my dotfiles using Chezmoi, then runs mh-install to install dev tools.
# Targets a Debian Docker container or tart Debian VM.
...
. "$HOME/.nix-profile/etc/profile.d/nix.sh" || fail "Failed to source Nix profile"
